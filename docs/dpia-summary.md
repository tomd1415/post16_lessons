...
- Health endpoint is public (`/api/health`). Prometheus metrics are available at `/metrics`. Admin-only JSON metrics are available at `/api/metrics` and `/api/admin/metrics`.
